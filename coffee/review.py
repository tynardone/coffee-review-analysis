...
fetched *or* parsed yields ``None`` rather than an exception, so that one
malformed page out of thousands cannot abort a full scrape.
"""
...
async def scrape_review(
    ...
    try:
        ...
        # A page whose markup doesn't match any expectation must not take the
        # whole run down with it. The caller awaits these one at a time, so an
        # exception escaping here aborts a 9,000-page scrape and writes nothing.
        # Returning None puts a parse failure on the same footing as a fetch
        # failure, which the caller already counts and reports.
